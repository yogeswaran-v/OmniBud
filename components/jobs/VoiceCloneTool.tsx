  ...
  return (
    <div style={{ maxWidth: 720, animation: 'fadeIn 0.25s ease' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={{ fontSize: 11, color: '#555', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Voice Sample</label>
        <div onClick={() => fileRef.current?.click()}
          style={{ border: `1.5px dashed ${sampleFile ? '#2a3a1a' : '#1e1e1e'}`, borderRadius: 12, padding: '28px 20px', textAlign: 'center', cursor: 'pointer', background: sampleFile ? '#0d150a' : '#0c0c0c', transition: 'all 0.2s' }}>
          ...
          {sampleFile ? (
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: '#555' }}>Drop audio file or click to upload</div>
              <div style={{ fontSize: 11, color: '#333', marginTop: 4 }}>WAV · MP3 · M4A · Free plan: 30 seconds max</div>
            </div>
          )}
        </div>
      </div>
