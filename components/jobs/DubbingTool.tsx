  ...
  return (
    <div style={{ maxWidth: 720, animation: 'fadeIn 0.25s ease' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 6 }}>Transcribe → translate → re-voice. Full lip-sync-aware pipeline powered by OmniVoice.</p>
      </div>

      {error && <div style={{ padding: '12px 16px', background: 'var(--danger-dim)', border: '1px solid var(--danger)', borderRadius: 10, fontSize: 13, color: '#e05555', marginBottom: 20 }}>{error}</div>}
      ...
      <div style={{ padding: '14px 16px', background: '#120e04', border: '1px solid #2a2010', borderRadius: 10, marginBottom: 24, display: 'flex', gap: 12 }}>
        ...
        <div>
          <div style={{ fontSize: 12, color: 'var(--warning)', fontWeight: 500 }}>Free plan: 5-minute video limit · Audio watermark applied</div>
          <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>Upgrade to Pro for 60-minute videos and clean exports.</div>
        </div>
      </div>

      {/* Video source */}
      <div style={{ marginBottom: 20 }}>
        <label style={{ fontSize: 11, color: 'var(--text-3)', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Video Source</label>
        <input value={videoUrl} onChange={e => setVideoUrl(e.target.value)}
          placeholder="Paste YouTube URL (e.g. https://youtube.com/watch?v=...)"
          style={{ width: '100%', padding: '12px 14px', fontSize: 13 }} />
        <div style={{ fontSize: 11, color: 'var(--text-4)', marginTop: 6 }}>YouTube URLs supported. Direct video upload coming soon.</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={s.step} style={{ padding: '14px', background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: 10 }}>
            <div style={{ fontSize: 10, color: 'var(--text-4)', marginBottom: 4 }}>Step {s.step}</div>
      ...
      {job?.status === 'failed' && <div style={{ marginTop: 16, padding: '14px', background: 'var(--danger-dim)', border: '1px solid var(--danger)', borderRadius: 10, fontSize: 13, color: '#e05555' }}>Job failed: {job.error}. Please try again.</div>}
    </div>
  )
}
