...
const QUOTE = { text: 'my voice, my accent â€” in every market we launched.', author: 'founder' }
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 380, animation: 'springIn 0.5s var(--ease-spring) both' }}>
        <div style={{ width: 56, height: 56, background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.25)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 22, color: 'var(--accent)' }}>âœ“</div>
        ...
        <Link href="/auth/login" style={{ display: 'inline-block', marginTop: 24, fontSize: 13, color: 'var(--accent)' }}>â† back to sign in</Link>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '55% 45%', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 56px', animation: 'fadeIn 0.4s var(--ease-out) both' }}>
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', background: 'rgba(249,115,22,0.07)', border: '1px solid rgba(249,115,22,0.20)', borderRadius: 20, marginBottom: 16 }}>
            <span style={{ fontSize: 10, color: 'var(--accent)', fontWeight: 700, letterSpacing: '0.06em' }}>FREE â€” 5 MIN/DAY INCLUDED</span>
          </div>
          ...
          <button onClick={handleSignup} disabled={loading || !email || !password}
            ...
            {loading ? <><div className="spinner" /> creating accountâ€¦</> : 'create free account â†’'}
          </button>

          <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-3)', marginTop: 22 }}>
            already have an account?{' '}
            <Link href="/auth/login" style={{ color: 'var(--accent)', fontWeight: 500 }}>sign in â†’</Link>
          </p>
        </div>
      </div>
